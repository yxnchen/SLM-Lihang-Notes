...
- 信息增益值得大小是相对于训练数据集而言的，没有绝对意义，在分类困难时（经验熵较大的时候），信息增益值会偏大；
- **信息增益比**（information gain ratio）可以进行校正，是特征选择的另一准则；
- **定义**：特征$A​$对训练数据集$T​$的信息增益比$g_R(T,A)​$定义为其信息增益$g(T,A)​$与训练集的经验熵$H(T)之比

$$
g_R(T,A)=\frac{g(T,A)}{H(T)}
$$
...
- **CART剪枝算法**：
  ...
  3. 选择$g(t)$值最小的结点剪枝$DT_t$，并对叶结点$t$以多数表决法决定其类，得到子树$DT_1$；

  4. 如果树$DT_1$不是由根结点单独构成的树，回到步骤3（此时树已经改变）递归；
